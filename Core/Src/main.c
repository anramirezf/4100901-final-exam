...
// Declare the ring buffer variables and variable for multiple fails
uint8_t multiple_fails = 0;
...
// Check if the sequence is equal to the data received in ring buffer
	...
	multiple_fails += 1;
// Check if they get wrong the sequence 3 times
	if (multiple_fails == 3){
	// Print block in case multiple fails
		ssd1306_Fill(Black);
		ssd1306_SetCursor(20, 20);
		ssd1306_WriteString("Blocked", Font_11x18, White);
		ssd1306_UpdateScreen();
		// Wait 30 seconds to get back to reading
		HAL_Delay(30000);
		// Reset multiple fails
		multiple_fails = 0;

}
else{
// If the get wrong the sequence one time
	ssd1306_Fill(Black);
	ssd1306_SetCursor(20, 20);
